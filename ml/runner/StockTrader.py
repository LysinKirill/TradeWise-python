...
class StockTrader:
    ...
    async def trading_cycle(self, current_candle: Candle):
        ...
        try:
            ...
            #    np.array(self.price_history).reshape(-1, 1))}")
            self.logger.info(f"Candle #{self.current_candle}: Current price = {current_price:.3f}, Predicted price = {pred_price:.3f}, expected return = {expected_return:.5f}. Timestamp = {current_candle.timestamp}" + ("" if trade_available else " [Trade unavailable]"))
            self.current_candle += 1
